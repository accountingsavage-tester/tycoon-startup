...
export default function Home(){
 ...
 return <main className={styles.game}>
  ...
  <section className={styles.world}><div className={styles.sky}><div className={styles.sun}/><div className={`${styles.cloud} ${styles.cloud1}`}/><div className={`${styles.cloud} ${styles.cloud2}`}/></div><div className={styles.city}><div className={`${styles.building} ${styles.b1}`}/><div className={`${styles.building} ${styles.b2}`}/><div className={`${styles.building} ${styles.b3}`}/><span className={styles.tree}/><span className={`${styles.tree} ${styles.tree2}`}/></div><div className={styles.road}/>
   <button className={styles.shopBuilding} onClick={()=>setScreen("shop")}><div className={styles.shopSign}>CORNER CAFÉ</div><div className={styles.shopWindow}>{g.product?<><ProductIcon type={g.product.icon}/><small>OPEN</small></>:<><b>+</b><small>EMPTY SHOP</small></>}</div><div className={styles.door}/></button>
   {g.customers>0&&Array.from({length:Math.min(9,g.customers)}).map((_,i)=><div className={styles.person} key={i} style={{left:`${16+i*9}%`,animationDelay:`${i*.2}s`}}><div className={styles.head}/><div className={styles.body}/></div>)}
   {g.product&&g.inventory>0&&<button className={styles.customerBubble} onClick={sell}><span>A customer is waiting</span><b><ProductIcon type={g.product.icon}/>{g.product.name}</b><strong>SELL +₱{g.product.price}</strong></button>}
  ...
  {screen==="shop"&&<div className={styles.overlay}><div className={styles.modal}><button className={styles.close} onClick={()=>setScreen("town")}><X/></button><small className={styles.kicker}>DAY {g.day} · FIRST DECISION</small><h1>Choose your first product</h1><p>You have ₱{g.cash.toLocaleString()}. Buy your first batch, attract customers, and make your first sale.</p><div className={styles.products}>{products.map(p=><button className={styles.product} key={p.id} onClick={()=>choose(p)}><span className={styles.productArt}><ProductIcon type={p.icon}/></span><b>{p.name}</b><small>Cost ₱{p.cost} · Sell ₱{p.price}</small><strong>Buy 10 · ₱{p.cost*10}</strong></button>)}</div></div></div>}
...
function ProductIcon({type}:{type:string}){return <span className={`${styles.icon} ${styles[type]}`} aria-hidden="true"/>}
